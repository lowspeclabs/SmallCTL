...
TOOL_CALL_PATTERN = re.compile(r"\b(file_read|file_write|file_patch|shell_exec|ssh_exec|" +
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Lint model_output.jsonl for common issues.")
    ...
    parser.add_argument("--repeated-phrases", action="store_true", help="Highlight repeated-phrase degeneration")
    parser.add_argument("--summary", action="store_true", help="Summary only (default if no flags)")
...
def _lint(run_dir: Path) -> dict[str, Any]:
    ...
    for rec in records:
        ...
        if event == "model_output_degenerate_loop" or event == "model_output_degenerate_loop_exhausted":
        ...
        if text.strip() and not TOOL_CALL_PATTERN.search(text):
            if len(text) > 20 and not event.startswith("model_token") and event != "model_thinking":
...
def _render_text(run_dir: Path, results: dict[str, Any], show_repeated: bool, show_summary: bool) -> str:
    lines: list[str] = []
    lines.append(colorize(f"Model output lint for {run_dir.name}", Colors.BOLD + Colors.CYAN))
    ...
    lines.append(f"  degenerate loops:                 {len(results['degenerate_loops'])}")
    lines.append(f"  thinking tags in assistant text:  {len(results['thinking_tags_in_assistant'])}")
    lines.append(f"  suspicious (no tool call syntax): {len(results['missing_tool_calls'])}")
    lines.append(f"  tool call syntax detected:        {len(results['tool_call_syntax_suspected'])}")
    ...
    if results["degenerate_loops"]:
        ...
        for d in results["degenerate_loops"][:10]:
            lines.append(f"  {d['timestamp']}  phrase={d['repeated_phrase'][:80]}  count={d['repeat_count']}  [{d['trace_id']}]")
    ...
    if results["thinking_tags_in_assistant"]:
        lines.append("")
        lines.append(colorize("Thinking tags leaked into assistant text", Colors.BOLD + Colors.YELLOW))
    ...
    if show_repeated and results["repeated_phrases"]:
        lines.append("")
        lines.append(colorize("Repeated identical assistant texts", Colors.BOLD + Colors.YELLOW))
...
def main() -> int:
    ...
    if args.json:
        print(json.dumps({"run_dir": str(run_dir), "run_name": run_dir.name, **results}, indent=2, default=str))
        return 0

    print(_render_text(run_dir, results, show_repeated=args.repeated_phrases, show_summary=show_summary))
    return 0
