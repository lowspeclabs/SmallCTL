...
def record_failure_outcome(service: Any, *, tool_name: str, result: Any) -> None:
    if not result.success and result.error:
        ...
        if metadata.get("reason") == "web_fetch_duplicate_result_id":
            _note_anchor(service.harness, content=f"{tool_name} duplicate fetch: {str(result.error).strip()}", tag="note")
            return
        if (
            not metadata.get("hallucination")
            and not metadata.get("approval_denied")
            and not metadata.get("suppress_failure_persistence")
        ):
