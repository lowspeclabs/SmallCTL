...
@pytest.mark.parametrize(
    ...
    (
        ...
        "sshpass -p hunter2 ssh root@192.0.2.10",
        "postgres://user:hunter2@db/app",
        'password is "hunter2"',
    ),
)
def test_passwords_are_not_redacted(text: str) -> None:
...
def test_c2_messages_preserve_password_in_non_json_tool_call_arguments() -> None:
    messages = [
        {
            "role": "assistant",
            "tool_calls": [
    ...
    redacted = redact_sensitive_messages(messages)
    assert "hunter2" in redacted[0]["tool_calls"][0]["function"]["arguments"]
