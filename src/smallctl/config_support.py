...
_logger = logging.getLogger("smallctl.config")


def _warn_on_permissive_dotenv(path: Path) -> None:
    """Warn when a dotenv file is group/other-readable without printing values."""
    try:
    ...
    if mode & 0o044:
        _logger.warning(
            "Dotenv file %s is readable by group/other users (mode %s); restrict it with chmod 600.",
            path,
            oct(mode & 0o777),
        )
...
def _read_dotenv(path: Path) -> dict[str, str]:
    if not path.exists():
        return {}
    _warn_on_permissive_dotenv(path)
    parsed: dict[str, str] = {}
...
def _normalize_run_mode(value: Any) -> str:
    ...
    return (
        mode
        if mode in {"auto", "chat", "loop", "planning", "indexer", "tool_plan"}
        else "auto"
    )
...
_BOOL_CONFIG_KEYS = {
    ...
    "langgraph_native_timeouts_enabled",
    "langgraph_native_retries_enabled",
    "langgraph_error_handlers_enabled",
    "langgraph_stream_events_enabled",
    "langgraph_cache_policy_enabled",
    "tool_plan_runtime_enabled",
    "tool_plan_auto_select",
    "tool_plan_readonly_only",
    "tool_plan_allow_web",
    "tool_plan_allow_artifact_read",
    "allow_artifact_read_large_context",
    "tool_plan_allow_git",
...
_INT_CONFIG_KEYS = {
    ...
    "langgraph_native_retry_max_attempts",
    ...
    "tool_call_repair_max_actions_per_call",
...
_COMMA_LIST_CONFIG_KEYS = {
    "test_time_scaling_runtimes",
...
def _apply_config_aliases(values: dict[str, Any]) -> None:
    ...
    if "graph_checkpointer" in values:
        values["graph_checkpointer"] = _normalize_graph_checkpointer(
            values["graph_checkpointer"]
        )
    ...
    if (
        "startup_grace_period_sec" not in values
        and "backend_restart_grace_sec" in values
    ):
...
def _env_raw_config(env_or_dotenv: Any) -> dict[str, Any]:
    return {
        ...
        "graph_model_call_timeout_sec": env_or_dotenv(
            f"{ENV_PREFIX}GRAPH_MODEL_CALL_TIMEOUT_SEC"
        ),
        "graph_dispatch_tools_timeout_sec": env_or_dotenv(
            f"{ENV_PREFIX}GRAPH_DISPATCH_TOOLS_TIMEOUT_SEC"
        ),
        "graph_idle_watchdog_sec": env_or_dotenv(
            f"{ENV_PREFIX}GRAPH_IDLE_WATCHDOG_SEC"
        ),
        "graph_recursion_limit": env_or_dotenv(f"{ENV_PREFIX}GRAPH_RECURSION_LIMIT"),
        "graph_coding_recursion_limit": env_or_dotenv(
            f"{ENV_PREFIX}GRAPH_CODING_RECURSION_LIMIT"
        ),
        "langgraph_native_timeouts_enabled": env_or_dotenv(
            f"{ENV_PREFIX}LANGGRAPH_NATIVE_TIMEOUTS_ENABLED"
        ),
        "langgraph_native_retries_enabled": env_or_dotenv(
            f"{ENV_PREFIX}LANGGRAPH_NATIVE_RETRIES_ENABLED"
        ),
        "langgraph_error_handlers_enabled": env_or_dotenv(
            f"{ENV_PREFIX}LANGGRAPH_ERROR_HANDLERS_ENABLED"
        ),
        "langgraph_stream_events_enabled": env_or_dotenv(
            f"{ENV_PREFIX}LANGGRAPH_STREAM_EVENTS_ENABLED"
        ),
        "langgraph_cache_policy_enabled": env_or_dotenv(
            f"{ENV_PREFIX}LANGGRAPH_CACHE_POLICY_ENABLED"
        ),
        "langgraph_native_retry_max_attempts": env_or_dotenv(
            f"{ENV_PREFIX}LANGGRAPH_NATIVE_RETRY_MAX_ATTEMPTS"
        ),
        "needs_human_timeout_sec": env_or_dotenv(
            f"{ENV_PREFIX}NEEDS_HUMAN_TIMEOUT_SEC"
        ),
        ...
        "staged_step_prompt_tokens": env_or_dotenv(
            f"{ENV_PREFIX}STAGED_STEP_PROMPT_TOKENS"
        ),
        "tool_plan_runtime_enabled": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_PLAN_RUNTIME_ENABLED"
        ),
        "tool_plan_auto_select": env_or_dotenv(f"{ENV_PREFIX}TOOL_PLAN_AUTO_SELECT"),
        "tool_plan_readonly_only": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_PLAN_READONLY_ONLY"
        ),
        "tool_plan_max_steps": env_or_dotenv(f"{ENV_PREFIX}TOOL_PLAN_MAX_STEPS"),
        "tool_plan_max_repair_attempts": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_PLAN_MAX_REPAIR_ATTEMPTS"
        ),
        "schema_validation_max_repair_attempts": env_or_dotenv(
            f"{ENV_PREFIX}SCHEMA_VALIDATION_MAX_REPAIR_ATTEMPTS"
        ),
        "tool_call_repair_enabled": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_CALL_REPAIR_ENABLED"
        ),
        "tool_call_repair_log_only": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_CALL_REPAIR_LOG_ONLY"
        ),
        "tool_call_repair_max_actions_per_call": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_CALL_REPAIR_MAX_ACTIONS_PER_CALL"
        ),
        "tool_plan_observation_token_limit": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_PLAN_OBSERVATION_TOKEN_LIMIT"
        ),
        "tool_plan_max_observation_chars_per_step": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_PLAN_MAX_OBSERVATION_CHARS_PER_STEP"
        ),
        "tool_plan_solver_fresh_output_limit": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_PLAN_SOLVER_FRESH_OUTPUT_LIMIT"
        ),
        ...
        "tool_plan_allow_artifact_read": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_PLAN_ALLOW_ARTIFACT_READ"
        ),
        "tool_plan_fallback_to_loop_on_invalid_plan": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_PLAN_FALLBACK_TO_LOOP_ON_INVALID_PLAN"
        ),
        ...
        "tool_dag_preserve_result_order": env_or_dotenv(
            f"{ENV_PREFIX}TOOL_DAG_PRESERVE_RESULT_ORDER"
        ),
        "solver_refine_enabled": env_or_dotenv(f"{ENV_PREFIX}SOLVER_REFINE_ENABLED"),
        "solver_refine_max_passes": env_or_dotenv(
            f"{ENV_PREFIX}SOLVER_REFINE_MAX_PASSES"
        ),
        "solver_refine_on_final_answer": env_or_dotenv(
            f"{ENV_PREFIX}SOLVER_REFINE_ON_FINAL_ANSWER"
        ),
        "solver_refine_on_patch_plan": env_or_dotenv(
            f"{ENV_PREFIX}SOLVER_REFINE_ON_PATCH_PLAN"
        ),
        "solver_refine_on_task_complete": env_or_dotenv(
            f"{ENV_PREFIX}SOLVER_REFINE_ON_TASK_COMPLETE"
        ),
        "solver_refine_token_budget": env_or_dotenv(
            f"{ENV_PREFIX}SOLVER_REFINE_TOKEN_BUDGET"
        ),
        "rewoo_lane_frames_enabled": env_or_dotenv(
            f"{ENV_PREFIX}REWOO_LANE_FRAMES_ENABLED"
        ),
        "rewoo_planner_frame_enabled": env_or_dotenv(
            f"{ENV_PREFIX}REWOO_PLANNER_FRAME_ENABLED"
        ),
        "rewoo_solver_frame_enabled": env_or_dotenv(
            f"{ENV_PREFIX}REWOO_SOLVER_FRAME_ENABLED"
        ),
        "rewoo_refiner_frame_enabled": env_or_dotenv(
            f"{ENV_PREFIX}REWOO_REFINER_FRAME_ENABLED"
        ),
        "rewoo_frame_token_budget": env_or_dotenv(
            f"{ENV_PREFIX}REWOO_FRAME_TOKEN_BUDGET"
        ),
        "test_time_scaling_enabled": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_ENABLED"
        ),
        "test_time_scaling_runtimes": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_RUNTIMES"
        ),
        "test_time_scaling_trigger": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_TRIGGER"
        ),
        "test_time_scaling_max_candidates": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_MAX_CANDIDATES"
        ),
        "test_time_scaling_min_candidates": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_MIN_CANDIDATES"
        ),
        "test_time_scaling_policy": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_POLICY"
        ),
        "test_time_scaling_strategy": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_STRATEGY"
        ),
        "test_time_scaling_score_threshold": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_SCORE_THRESHOLD"
        ),
        "test_time_scaling_parallel_max": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_PARALLEL_MAX"
        ),
        "test_time_scaling_timeout_sec": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_TIMEOUT_SEC"
        ),
        "test_time_scaling_mutating_parallel_enabled": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_MUTATING_PARALLEL_ENABLED"
        ),
        "test_time_scaling_all_fail_action": env_or_dotenv(
            f"{ENV_PREFIX}TEST_TIME_SCALING_ALL_FAIL_ACTION"
        ),
        ...
        "escalation_auto_trigger": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_AUTO_TRIGGER"
        ),
        ...
        "escalation_provider_profile": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_PROVIDER_PROFILE"
        ),
        ...
        "escalation_chat_endpoint": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_CHAT_ENDPOINT"
        ),
        "escalation_max_prompt_chars": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_MAX_PROMPT_CHARS"
        ),
        "escalation_max_response_tokens": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_MAX_RESPONSE_TOKENS"
        ),
        ...
        "escalation_max_per_task": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_MAX_PER_TASK"
        ),
        "escalation_cooldown_turns": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_COOLDOWN_TURNS"
        ),
        "escalation_repeated_failure_threshold": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_REPEATED_FAILURE_THRESHOLD"
        ),
        "escalation_require_tool_plan_evidence": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_REQUIRE_TOOL_PLAN_EVIDENCE"
        ),
        "escalation_redact_secrets": env_or_dotenv(
            f"{ENV_PREFIX}ESCALATION_REDACT_SECRETS"
        ),
        ...
        "swa_prompt_cap": env_or_dotenv(f"{ENV_PREFIX}SWA_PROMPT_CAP"),
        "max_completion_tokens": env_or_dotenv(f"{ENV_PREFIX}MAX_COMPLETION_TOKENS"),
        "max_prompt_tokens_explicit": env_or_dotenv(
            f"{ENV_PREFIX}MAX_PROMPT_TOKENS_EXPLICIT"
        ),
        "reserve_completion_tokens": env_or_dotenv(
            f"{ENV_PREFIX}RESERVE_COMPLETION_TOKENS"
        ),
        "reserve_tool_tokens": env_or_dotenv(f"{ENV_PREFIX}RESERVE_TOOL_TOKENS"),
        "first_token_timeout_sec": env_or_dotenv(
            f"{ENV_PREFIX}FIRST_TOKEN_TIMEOUT_SEC"
        ),
        "healthcheck_url": env_or_dotenv(f"{ENV_PREFIX}HEALTHCHECK_URL"),
        "restart_command": env_or_dotenv(f"{ENV_PREFIX}RESTART_COMMAND"),
        "startup_grace_period_sec": env_or_dotenv(
            f"{ENV_PREFIX}STARTUP_GRACE_PERIOD_SEC"
        ),
        "max_restarts_per_hour": env_or_dotenv(f"{ENV_PREFIX}MAX_RESTARTS_PER_HOUR"),
        "backend_healthcheck_url": env_or_dotenv(
            f"{ENV_PREFIX}BACKEND_HEALTHCHECK_URL"
        ),
        "backend_restart_command": env_or_dotenv(
            f"{ENV_PREFIX}BACKEND_RESTART_COMMAND"
        ),
        "backend_unload_command": env_or_dotenv(f"{ENV_PREFIX}BACKEND_UNLOAD_COMMAND"),
        "backend_healthcheck_timeout_sec": env_or_dotenv(
            f"{ENV_PREFIX}BACKEND_HEALTHCHECK_TIMEOUT_SEC"
        ),
        "backend_restart_grace_sec": env_or_dotenv(
            f"{ENV_PREFIX}BACKEND_RESTART_GRACE_SEC"
        ),
        ...
        "artifact_snippet_token_limit": env_or_dotenv(
            f"{ENV_PREFIX}ARTIFACT_SNIPPET_TOKEN_LIMIT"
        ),
        "allow_artifact_read_large_context": env_or_dotenv(
            f"{ENV_PREFIX}ALLOW_ARTIFACT_READ_LARGE_CONTEXT"
        ),
        "multi_file_artifact_snippet_limit": env_or_dotenv(
            f"{ENV_PREFIX}MULTI_FILE_ARTIFACT_SNIPPET_LIMIT"
        ),
        "multi_file_primary_file_limit": env_or_dotenv(
            f"{ENV_PREFIX}MULTI_FILE_PRIMARY_FILE_LIMIT"
        ),
        "remote_task_artifact_snippet_limit": env_or_dotenv(
            f"{ENV_PREFIX}REMOTE_TASK_ARTIFACT_SNIPPET_LIMIT"
        ),
        "remote_task_primary_file_limit": env_or_dotenv(
            f"{ENV_PREFIX}REMOTE_TASK_PRIMARY_FILE_LIMIT"
        ),
        ...
        "artifact_summarization_threshold": env_or_dotenv(
            f"{ENV_PREFIX}ARTIFACT_SUMMARIZATION_THRESHOLD"
        ),
        "chunk_mode_min_bytes": env_or_dotenv(f"{ENV_PREFIX}CHUNK_MODE_MIN_BYTES"),
        "chunk_mode_new_file_only": env_or_dotenv(
            f"{ENV_PREFIX}CHUNK_MODE_NEW_FILE_ONLY"
        ),
        "chunk_mode_supported_models": env_or_dotenv(
            f"{ENV_PREFIX}CHUNK_MODE_SUPPORTED_MODELS"
        ),
        "small_model_soft_write_chars": env_or_dotenv(
            f"{ENV_PREFIX}SMALL_MODEL_SOFT_WRITE_CHARS"
        ),
        "small_model_hard_write_chars": env_or_dotenv(
            f"{ENV_PREFIX}SMALL_MODEL_HARD_WRITE_CHARS"
        ),
        "new_file_chunk_mode_line_estimate": env_or_dotenv(
            f"{ENV_PREFIX}NEW_FILE_CHUNK_MODE_LINE_ESTIMATE"
        ),
        "allow_multi_section_turns_for_small_edits": env_or_dotenv(
            f"{ENV_PREFIX}ALLOW_MULTI_SECTION_TURNS_FOR_SMALL_EDITS"
        ),
        "failed_local_patch_limit": env_or_dotenv(
            f"{ENV_PREFIX}FAILED_LOCAL_PATCH_LIMIT"
        ),
        "enable_write_intent_recovery": env_or_dotenv(
            f"{ENV_PREFIX}ENABLE_WRITE_INTENT_RECOVERY"
        ),
        "enable_assistant_code_write_recovery": env_or_dotenv(
            f"{ENV_PREFIX}ENABLE_ASSISTANT_CODE_WRITE_RECOVERY"
        ),
        "write_recovery_min_confidence": env_or_dotenv(
            f"{ENV_PREFIX}WRITE_RECOVERY_MIN_CONFIDENCE"
        ),
        "write_recovery_allow_raw_text_targets": env_or_dotenv(
            f"{ENV_PREFIX}WRITE_RECOVERY_ALLOW_RAW_TEXT_TARGETS"
        ),
        "loop_guard_enabled": env_or_dotenv(f"{ENV_PREFIX}LOOP_GUARD_ENABLED"),
        "loop_guard_stagnation_threshold": env_or_dotenv(
            f"{ENV_PREFIX}LOOP_GUARD_STAGNATION_THRESHOLD"
        ),
        "loop_guard_level2_threshold": env_or_dotenv(
            f"{ENV_PREFIX}LOOP_GUARD_LEVEL2_THRESHOLD"
        ),
        "loop_guard_recent_writes_limit": env_or_dotenv(
            f"{ENV_PREFIX}LOOP_GUARD_RECENT_WRITES_LIMIT"
        ),
        "loop_guard_tail_lines": env_or_dotenv(f"{ENV_PREFIX}LOOP_GUARD_TAIL_LINES"),
        "loop_guard_similarity_threshold": env_or_dotenv(
            f"{ENV_PREFIX}LOOP_GUARD_SIMILARITY_THRESHOLD"
        ),
        "loop_guard_cumulative_write_gate": env_or_dotenv(
            f"{ENV_PREFIX}LOOP_GUARD_CUMULATIVE_WRITE_GATE"
        ),
        "loop_guard_checkpoint_gate": env_or_dotenv(
            f"{ENV_PREFIX}LOOP_GUARD_CHECKPOINT_GATE"
        ),
        "loop_guard_diff_gate": env_or_dotenv(f"{ENV_PREFIX}LOOP_GUARD_DIFF_GATE"),
        "max_consecutive_errors": env_or_dotenv(f"{ENV_PREFIX}MAX_CONSECUTIVE_ERRORS"),
        ...
        "fama_max_active_mitigations": env_or_dotenv(
            f"{ENV_PREFIX}FAMA_MAX_ACTIVE_MITIGATIONS"
        ),
        "fama_signal_window": env_or_dotenv(f"{ENV_PREFIX}FAMA_SIGNAL_WINDOW"),
        "fama_done_gate_on_failure": env_or_dotenv(
            f"{ENV_PREFIX}FAMA_DONE_GATE_ON_FAILURE"
        ),
        "fama_capsule_token_budget": env_or_dotenv(
            f"{ENV_PREFIX}FAMA_CAPSULE_TOKEN_BUDGET"
        ),
        "fama_llm_judge_enabled": env_or_dotenv(f"{ENV_PREFIX}FAMA_LLM_JUDGE_ENABLED"),
        "fama_llm_judge_min_severity": env_or_dotenv(
            f"{ENV_PREFIX}FAMA_LLM_JUDGE_MIN_SEVERITY"
        ),
        ...
        "reflexion_persist_cross_task": env_or_dotenv(
            f"{ENV_PREFIX}REFLEXION_PERSIST_CROSS_TASK"
        ),
        "reflexion_min_failure_severity": env_or_dotenv(
            f"{ENV_PREFIX}REFLEXION_MIN_FAILURE_SEVERITY"
        ),
        ...
        "subtask_inject_completed_limit": env_or_dotenv(
            f"{ENV_PREFIX}SUBTASK_INJECT_COMPLETED_LIMIT"
        ),
        "sudo_password": env_or_dotenv(f"{ENV_PREFIX}SUDO_PASSWORD"),
