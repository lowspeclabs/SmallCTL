...
async def run_single_trial(
    ...
    try:
        ...
        try:
            ...
            teardown_done = await _teardown_harness(harness, logger)
        ...
        if cfg.get("generate_test_on_crash"):
            try:
                test_file = _write_crash_reproduction(
                    exc=exc,
                    traceback_text=tb,
                    harness=harness,
                    cfg=cfg,
...
async def _teardown_harness(harness: Any, logger: AHOLogger) -> bool:
    if harness is None:
        return False
    try:
...
def _write_crash_reproduction(
    *,
    ...
    test_file = test_dir / f"test_crash_{type(exc).__name__.lower()}_{uuid.uuid4().hex[:6]}.py"
    state_dump = {}
    if harness is not None and hasattr(harness, "state"):
        state = getattr(harness, "state")
        if hasattr(state, "to_dict"):
            state_dump = state.to_dict()
        else:
            state_dump = {"scratchpad": getattr(state, "scratchpad", {})}

    test_code = f'''# Auto-generated by `python -m aho.harness_runner --generate-failing-test`.
from __future__ import annotations
...
CRASH_TYPE = {type(exc).__name__!r}
CRASH_MESSAGE = {str(exc)!r}
TRACEBACK = {traceback_text!r}
TRIAL_ID = {trial_id!r}
STRATEGY_ID = {cfg.get("strategy_id")!r}
STATE_DUMP = {state_dump!r}


def test_auto_generated_crash_state_is_loadable() -> None:
    state = LoopState.from_dict(STATE_DUMP) if STATE_DUMP else LoopState()
    assert state is not None
    assert CRASH_TYPE


def test_auto_generated_crash_reproducer() -> None:
    pytest.fail(
        f"Reproducing {{CRASH_TYPE}} in trial {{TRIAL_ID}} "
        f"(strategy={{STRATEGY_ID}}): {{CRASH_MESSAGE}}\\n\\n{{TRACEBACK}}"
    )
'''
