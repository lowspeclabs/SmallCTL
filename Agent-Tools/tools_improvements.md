...
| ID | Tool | Priority | Description | Motivation | Status |
| IMP-017 | session_replay.py, run_diagnose.py | high | Add a compact command timeline that includes step/index, command, effective exit status, and the first semantic error line; flag masking constructs such as `|| true`, `2>&1 | head`, and `cmd1; cmd2` when output contains errors or expected postconditions disappear. | In run `16c99e84`, all 49 SSH dispatches appeared successful even though resolver probes failed and a piped `docker compose up` left the stack removed. A semantic timeline would make this failure obvious without manual `jq` extraction. | open |
| IMP-016 | trace_call.py, session_replay.py, run_diagnose.py | high | Add `--session-id` selection and print a session-epoch summary whenever a run directory contains multiple `session_id` values; default aggregate reports should show per-session task/tool totals rather than merging reused task IDs. | TUI restarts/reinitializations can append multiple session prefixes to one directory. Explicit epoch handling would make cancellation, confirmation-gate, and FAMA RCAs reliable without manual JSONL filtering. | open |
| IMP-014 | run_diagnose.py, logwatch.py | medium | Detect and surface `artifact_read_exposure_disabled` more specifically. When `artifact_read` is hidden, include the resolved reason (e.g., `large_context_window` vs. `large_model`) and the effective context-window/model values, and suggest checking whether the configured prompt budget is being ignored. | Today a `policy_block` classification is correct but generic; agents have to manually trace the `artifact_read_exposure_disabled` records to realize the tool was hidden because the raw server context limit (1M) dwarfed the configured `max_prompt_tokens` (32k), leaving the model unable to read its own truncated shell output. A specific classification/note would shortcut the next RCA. | open |
...
|----|------|---------|-----------|
| IMP-018 | run_diagnose.py, logwatch.py | Added a shared conservative `ssh_file_patch` divergence detector requiring a later same-host Docker container read of the same basename containing the old target but not its replacement, at least two successful reloads, and a subsequent HTTP 5xx. Diagnosis now classifies this before success/recovery failure, both tools expose structured evidence, and reported verification is preserved while effective verification is invalidated. | 2026-07-21 |
| IMP-015 | logwatch.py, run_diagnose.py | Added `detect_phantom_code_changes()` to agent_tools_lib.py: cross-checks `challenge_progress.last_code_change_paths` against the filesystem, flags paths missing on disk, detects staged-only write evidence from dispatch metadata or output text, and lists exact staging copies under `.smallctl/write_sessions/` using the harness's `{session}__{stem}__{label}{suffix}` naming convention (no substring false positives). Both tools render a "Phantom code changes" section; run_diagnose also adds recommended next steps. Regression tests added. | 2026-07-17 |
