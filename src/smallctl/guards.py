...
_SMALL_MODEL_SIZE_RE = re.compile(
    r"(?<![xX])\b(?P<size>\d+(?:\.\d+)?)b\b", re.IGNORECASE
)
...
def _repeated_action_directive_hint(state: LoopState, tool_name: str) -> str:
    ...
    if tool_name == "shell_exec":
        return "Directive Hint: use the existing command output or artifact; only run a different focused command if it will produce new evidence."
    return f"Directive Hint: stop repeating `{tool_name}` with identical outcome; choose a different concrete next action or complete the task."
...
def apply_triple_answer_guard(
    ...
    for call in pending_tool_calls:
        if call.tool_name in ("task_complete", "task_fail"):
            ...
            if ratio >= threshold and _assistant_and_message_are_similarly_scoped(
...
def _assistant_and_message_are_similarly_scoped(
    assistant_text: str, message: str
) -> bool:
