...
def test_task_classification_rules_table_preserves_precedence() -> None:
    ...
    # Precedence: local_execute > local_tool_remote_api > hybrid_execute > remote_execute > chat > plan_only > debug_inspect > analysis
    ...
    assert _TASK_CLASSIFICATION_RULES[3].mode == "local_execute"
    assert _TASK_CLASSIFICATION_RULES[4].mode == "hybrid_execute"
    assert _TASK_CLASSIFICATION_RULES[5].mode == "remote_execute"
    assert _TASK_CLASSIFICATION_RULES[6].mode == "local_execute"
    assert _TASK_CLASSIFICATION_RULES[7].mode == "chat"
    assert _TASK_CLASSIFICATION_RULES[8].mode == "plan_only"
    assert _TASK_CLASSIFICATION_RULES[9].mode == "remote_execute"
    assert _TASK_CLASSIFICATION_RULES[10].mode == "local_execute"
    assert _TASK_CLASSIFICATION_RULES[11].mode == "debug_inspect"
    assert _TASK_CLASSIFICATION_RULES[12].mode == "local_execute"
    assert _TASK_CLASSIFICATION_RULES[13].mode == "analysis"
    # Default fallback
