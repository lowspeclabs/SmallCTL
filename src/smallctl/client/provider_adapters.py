"""Provider-specific message adapters and sanitization."""
...
def sanitize_message_for_transport(message: dict[str, Any]) -> dict[str, Any]:
    """Sanitize a message for transport, keeping only standard fields."""
...
def sanitize_messages_with_pending_tool_cleanup(
    ...
    *,
    ...
    """Sanitize messages and clean up pending tool call blocks.
    
    Removes tool_calls from assistant messages when the corresponding
    tool messages are missing.
    """
        ...
        if role == "tool":
            ...
            if pending_tool_call_ids and tool_call_id and tool_call_id in pending_tool_call_ids:
                ...
                if rewrite_orphan_tool_messages:
                    sanitized.append(_rewrite_orphan_tool_message_for_openrouter(message))
                else:
                    sanitized.append(sanitize_message_for_transport(message))
            continue
...
def sanitize_messages_for_openrouter(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Sanitize messages for OpenRouter compatibility.
    
    Rewrites orphan tool messages to user messages for compatibility.
    """
...
def sanitize_messages_for_lmstudio(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Sanitize messages for LM Studio compatibility.
    
    Keeps LM Studio payloads chat-shaped and ends with a real user query.
...
def _rewrite_orphan_tool_message_for_openrouter(message: dict[str, Any]) -> dict[str, Any]:
    """Rewrite an orphan tool message as a user message for OpenRouter."""
...
def should_retry_without_stream_options(exc: Any) -> bool:
    """Check if an HTTP error indicates we should retry without stream_options.
    
    Some providers don't support stream_options or include_usage fields.
    """
    try:
