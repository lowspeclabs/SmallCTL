...
def _handle_add(args: argparse.Namespace, warm_store: ExperienceStore, cold_store: ExperienceStore) -> int:
    ...
    for m in all_memories:
        ...
        target.upsert(m)
        print(json.dumps({"status": "added", "memory_id": m.memory_id, "tier": m.tier}))
...
def _handle_list(args: argparse.Namespace, warm_store: ExperienceStore, cold_store: ExperienceStore) -> int:
    ...
    print(json.dumps([json_safe_value(m) for m in combined], indent=2))
    return 0
...
def _handle_search(args: argparse.Namespace, warm_store: ExperienceStore, cold_store: ExperienceStore) -> int:
    ...
    print(json.dumps([json_safe_value(m) for m in matches], indent=2))
    return 0
...
def _handle_forget(args: argparse.Namespace, warm_store: ExperienceStore, cold_store: ExperienceStore) -> int:
    ...
    target.delete(args.id)
    print(json.dumps({"status": "deleted", "memory_id": args.id, "tier": args.tier}))
    return 0

def _handle_promote(args: argparse.Namespace, warm_store: ExperienceStore, cold_store: ExperienceStore) -> int:
    m = warm_store.get(args.id)
    if not m:
        print(json.dumps({"status": "failed", "reason": f"Memory not found in warm tier: {args.id}"}))
        return 1
    m.tier = "cold"
    cold_store.upsert(m)
    warm_store.delete(args.id)
    print(json.dumps({"status": "promoted", "memory_id": args.id}))
    return 0
